...
export async function startStage2(projectId: string, prompt: string, vertices: any) {
  const requestUrl = encodeURI(BASE_URL + '/startStage2');
  const axiosResponse = await axios.post(requestUrl, { projectId, prompt, vertices });
